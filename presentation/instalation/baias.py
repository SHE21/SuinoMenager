import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QGraphicsView, QGraphicsScene, QGraphicsRectItem, QGraphicsTextItem, QPushButton
...
class GranjaWindow(QMainWindow):
    def __init__(self):
        ...
        self.baia = self.scene.addRect(50, 50, self.largura, self.altura, QPen(QColor(0, 0, 0)), QBrush(QColor(200, 200, 255)))
        ...
        dimens = 44
        self.texto.setPos(dimens + dimens / 2 - self.texto.boundingRect().width() / 2, dimens + dimens / 2 - self.texto.boundingRect().height() / 2)
    ...
    def alterar_baia(self):
        ...
        self.texto.setPos(50 + self.largura / 2 - self.texto.boundingRect().width() / 2, 50 + self.altura / 2 - self.texto.boundingRect().height() / 2)
